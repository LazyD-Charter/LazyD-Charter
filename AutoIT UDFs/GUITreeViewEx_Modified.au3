...
; This is a modified version of Melba23's work by LazyD (https://www.autoitscript.com/forum/profile/113124-gokay/). Look for #forcedef lines to find the added lines. Purpose of alteration: Find Total Size of Checked Files in a TreeView.
...
#forcedef $g_aTV_Event, $g_iTotalSizeBytes

					Local $aTV = $g_aTV_Event[0]
					if GUICtrlGetHandle($aTV[0][3]) = $hTV then
						;$aTV = $g_aTV_Event[0]
					Else
						$aTV = $g_aTV_Event[1]
					EndIf
					Local $iAlteredIndex = _ArraySearch($aTV,$g_GTVEx_hItemSelected,Default,Default,Default,Default,Default,5,False)
					Local $iSign = ($bState=True) ? (1) : (-1)
					$g_iTotalSizeBytes = ($aTV[$iAlteredIndex][0] = 0) ? ($g_iTotalSizeBytes + $iSign * $aTV[$iAlteredIndex][15]) : ($g_iTotalSizeBytes) ; if file
	...
	For $iIndex = 1 To $g_GTVEx_aTVData[0][0]
		If $hTV = $g_GTVEx_aTVData[$iIndex][0] Then
			...
			For $i = 0 To UBound($aTVData) - 1
				; Adjust item
				;_GUICtrlTreeView_SetChecked($hTV, $aTVData[$i][0], $bState) ; Comment this out
...
;~ 					$aTV = $g_aTV_Event[1]
;~ 				EndIf
;~ 				Local $iAlteredIndex = _ArraySearch($aTV,$aTVData[$i][0],Default,Default,Default,Default,Default,5,False)
;~ 				Local $iSign = ($bState=True) ? (1) : (-1)
;~ 				$g_iTotalSizeBytes = ($aTV[$iAlteredIndex][0] = 0) ? ($g_iTotalSizeBytes + $iSign * $aTV[$iAlteredIndex][15]) : ($g_iTotalSizeBytes) ; if file
...
#forcedef $g_aTV_Event, $g_iTotalSizeBytes
		...
		Else
			$aTV = $g_aTV_Event[1]
		EndIf
		Local $iAlteredIndex = _ArraySearch($aTV,$hChild,Default,Default,Default,Default,Default,5,False)
		Local $iSign = ($bState=True) ? (1) : (-1)
		$g_iTotalSizeBytes = ($aTV[$iAlteredIndex][0] = 0) ? ($g_iTotalSizeBytes + $iSign * $aTV[$iAlteredIndex][15]) : ($g_iTotalSizeBytes) ; if file
